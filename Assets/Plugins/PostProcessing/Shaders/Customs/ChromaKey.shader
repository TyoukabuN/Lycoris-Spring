Shader "X_Shader/PostProcess/ChromaKey"
{
	HLSLINCLUDE
        ...
        half4 ChromaKey(VaryingsDefault i) : SV_Target
        {
            ...
            // Unfeathered mask
            float mask = maskedTex2D(i.texcoord);

            // Feathering & smoothing
            float c = mask;
            float r = maskedTex2D( i.texcoord + pixelWidth);
            float l = maskedTex2D(i.texcoord - pixelWidth);
            float d = maskedTex2D(i.texcoord + pixelHeight);
            float u = maskedTex2D(i.texcoord - pixelHeight);
            float rd = maskedTex2D(i.texcoord + pixelWidth + pixelHeight) * .707;
            float dl = maskedTex2D(i.texcoord - pixelWidth + pixelHeight) * .707;
            float lu = maskedTex2D(i.texcoord - pixelHeight - pixelWidth) * .707;
            float ur = maskedTex2D(i.texcoord + pixelWidth - pixelHeight) * .707;
